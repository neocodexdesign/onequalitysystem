...
{  
    ...
    public function fetchEvents(array $fetchInfo): array
    {
        ...
        return Task::where('start', '>=', $fetchInfo['start'])
            ...
            ->map(function (Task $task) {
                $dateOnlyEvent =  Carbon::parse($task->start)->format('Y-m-d');
                
                return [
                    'id'    => $task->id,
                    'title' => $task->title . ' - Summary: ' . $task->summary . ' - Building:' . $task->location,
                    'start' => $dateOnlyEvent,
                    'color' => '#B39DDB',
                ];
            })
    ...
    public function fetchEventsx(array $fetchInfo): array
    {
